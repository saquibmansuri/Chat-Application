...
# This function will read env file line by line and store the output which will be used later
load_env_file() {
  ENV_FILE=$1
...
# This function will compare existing and new env variables and will store the environment variables that are no longer needed as an output which will be used later
...
update_service () {
  ...
  if [ $BLUE_REPLICAS -gt 0 ]; then
    ...
    #sudo docker service update --image "${NEW_IMAGE}" "${NEW_ENV_VARS}" "${ENV_VARS_TO_REMOVE}" --with-registry-auth --force "${STACK_NAME}_${SERVICE_NAME}_green"
    ...
    ENV_VARS_TO_REMOVE=$(get_env_vars_to_remove "$CURRENT_ENV_VARS" "$NEW_ENV_VARS")
    #sudo docker service update --image "${NEW_IMAGE}" "${NEW_ENV_VARS}" "${ENV_VARS_TO_REMOVE}" --with-registry-auth --force "${STACK_NAME}_${SERVICE_NAME}_blue"
    cmd="sudo docker service update --with-registry-auth --force --image"
    cmd="$cmd $NEW_IMAGE"
    cmd="$cmd $NEW_ENV_VARS"
    cmd="$cmd $ENV_VARS_TO_REMOVE"
